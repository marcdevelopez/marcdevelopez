# 👋 Hi! I'm Marcos García

I’m a **Mobile App Developer** specialized in **Flutter**, focused on building cross-platform solutions that connect people, optimize processes, and provide an excellent user experience. I also have a strong interest in **cybersecurity** and **clean development practices**.

---

## 🚀 About Me

- 🎓 Higher Technician in Cross-Platform Application Development (DAM)
- 📱 Focused on Flutter + Firebase, with personal projects that integrate:
  - Geolocation
  - Real-time chat
  - Maps and push notifications
- 💬 Looking to collaborate with teams that value quality, continuous improvement, and commitment

---

## 🛠️ Technologies and Tools
<br>

<p align="center">
  <a href="https://flutter.dev"><img src="https://img.shields.io/badge/-Flutter-02569B?logo=flutter&logoColor=white" /></a>
  <a href="https://dart.dev"><img src="https://img.shields.io/badge/-Dart-0175C2?logo=dart&logoColor=white" /></a>
  <a href="https://firebase.google.com"><img src="https://img.shields.io/badge/-Firebase-FFCA28?logo=firebase&logoColor=black" /></a>
  <a href="https://git-scm.com"><img src="https://img.shields.io/badge/-Git-F05032?logo=git&logoColor=white" /></a>
  <a href="https://github.com"><img src="https://img.shields.io/badge/-GitHub-181717?logo=github&logoColor=white" /></a>
  <a href="https://code.visualstudio.com"><img src="https://img.shields.io/badge/-VS%20Code-007ACC?logo=visual-studio-code&logoColor=white" /></a>
</p>


---

## 📂 Featured Repositories

- 🔗 [flutter_map_experiments](https://github.com/marcdevelopez/flutter_map_experiments) *(in progress)* – personal practice with maps, routes, and geolocation.
- 🚗 [ShuttleBiz](https://github.com/marcdevelopez/shuttlebiz) *(in progress)* – App to coordinate ridesharing among coworkers, with chat, geolocation, and real-time notifications.
- 💡 [SynFeel](https://github.com/marcdevelopez/synfeel) *(in progress)* – AI-powered emotional app designed to analyze user mood through journal entries.

---

## 📬 How to Reach Me
<br>
<p align="center">
  <a href="https://github.com"><img src="https://img.shields.io/badge/-GitHub-181717?logo=github&logoColor=white" /></a>
  <br><br>
  <a href="https://www.linkedin.com/in/marc-garcia-developez/"><img src="https://img.shields.io/badge/-LinkedIn-0A66C2?logo=linkedin&logoColor=white" /></a>
  <br><br>
  <a href="https://www.youtube.com/@marc_developez"><img src="https://img.shields.io/badge/-YouTube-FF0000?logo=youtube&logoColor=white" /></a>
</p>




---

> 💡 “Build technology with purpose. Learn, share, and improve, every day.”

